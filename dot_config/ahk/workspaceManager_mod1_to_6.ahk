...
Gui(){
	...
	Version=0.001.6
	Gui,Show,Hide,Workspace %version%
	PopulateGroups(),tv()
	OnExit,Exit
...
Hotkey(){
	...
	while,ww:=windows.item[A_Index-1],ea:=xml.ea(ww){
		if(WinActive(ea.title " ahk_class " getvalue(ww,"Class"))){
			...
			WinGet,max,minmax,% ea.title " ahk_class " getvalue(ww,"Class")
			toMaximize := max
			;MsgBox %toMaximize%
			;MsgBox %x% %y% %w% %h% %position%
	...
	while,ww:=windows.item[windows.length-(A_Index)],ea:=xml.ea(ww){
		if(minimized){
			...
			if(getvalue(ww,"Maximize")){
				...
				;maVarH := pos.h
				;maVarTitle := ea.title
				;MsgBox %maVarX% %maVarY% %maVarW% %maVarH% %maVarTitle%
