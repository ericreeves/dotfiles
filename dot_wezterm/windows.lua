...
function M.options(config)
	...
	-- config.default_gui_startup_args = { "connect", "SSHMUX:192.168.1.4" }
	...
	config.launch_menu = {
		...
		{
			label = " NeoVim Config",
			args = { "pwsh", "-NoExit", "-Command", "cd $HOME/.config/nvim && nvim" },
		},
		{
			label = " Dotfiles",
			args = { "pwsh", "-NoExit", "-Command", "cd $HOME/.dotfiles && nvim" },
		},
	}
end
